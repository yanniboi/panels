...
class PanelsStorageTest extends UnitTestCase {
  ...
  protected function setUp() {
    ...
    $this->displayVariant = $this->prophesize(DisplayVariantInterface::class);
    $this->displayVariant->getVariantPlugin()->willReturn($this->panelsDisplay->reveal());

    $this->displayVariantNotPanels = $this->prophesize(DisplayVariantInterface::class);
    $this->displayVariantNotPanels->getContexts()->shouldNotBeCalled();

    $non_panels_variant = $this->prophesize(HttpStatusCodeDisplayVariant::class);
    $this->displayVariantNotPanels->getVariantPlugin()->willReturn($non_panels_variant->reveal());

    $this->storage = $this->prophesize(EntityStorageInterface::class);

    $this->entityTypeManager = $this->prophesize(EntityTypeManagerInterface::class);
    $this->entityTypeManager->getStorage('display_variant')->willReturn($this->storage->reveal());
  }
  ...
  public function testLoad() {
    ...
    $this->displayVariant->getContexts()->willReturn([]);
    $this->panelsDisplay->setContexts([])->shouldBeCalledTimes(1);

    $this->storage->load('id_exists')->willReturn($this->displayVariant->reveal());
  ...
  public function testSaveSuccessful() {
    ...
    $this->displayVariant->save()->shouldBeCalledTimes(1);
  ...
  public function testSaveDoesntExist() {
    ...
    $this->displayVariant->save()->shouldNotBeCalled();
  ...
  public function testAccess() {
    ...
    $this->displayVariant->access('read', $account->reveal(), TRUE)->willReturn(AccessResult::allowed());
    ...
    $this->displayVariant->access('update', $account->reveal(), TRUE)->willReturn(AccessResult::allowed());
